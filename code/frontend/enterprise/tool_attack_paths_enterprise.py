...
try:
    from tool_threat_mapper_enterprise import (
    ...
    DEFAULT_BACKEND    = (_sd / "../backend/parse_attack_graph_enterprise.py").resolve()
    DEFAULT_ASSET_MAP  = (_sd / "../backend/threat_library/enterprise/asset_to_threats_enterprise.json").resolve()
    DEFAULT_THREAT_MAP = (_sd / "../backend/threat_library/enterprise/threat_to_tactic_enterprise.json").resolve()
    DEFAULT_AV_MAP     = (_sd / "../backend/threat_library/enterprise/attack_vector_feasibility_enterprise.json").resolve()
    DEFAULT_DEP_MAP    = (_sd / "../backend/threat_library/enterprise/dependency_enterprise.json").resolve()
    DEFAULT_IMPACT_MAP = (_sd / "../backend/threat_library/enterprise/impact_map_enterprise.json").resolve()
...
def _draw_top_banner(canvas: tk.Canvas, title: str, subtitle: str) -> None:
    ...
    for x in range(0, w, 52):
        canvas.create_line(x, 0, x, h, fill="#183653")
    for y in range(0, h, 26):
        canvas.create_line(0, y, w, y, fill="#183653")
    ...
    for x in (building_x, cloud_x, lock_x):
        canvas.create_oval(
            x - 22, cy + 24, x + 22, cy + 29,
            outline=glow_color,
            width=1
        )
    ...
    for row_y in (cy - 11, cy - 1):
        for col_x in (building_x - 12, building_x, building_x + 12):
            canvas.create_rectangle(
                col_x - 2.5, row_y - 2.5,
                col_x + 2.5, row_y + 2.5,
                outline=icon_line,
...
_DEFAULT_UKC = {
    "In": [
        ...
        "Persistence", "Stealth", "Defense Impairment", "Command and Control"
    ],
    "Through": [
        "Discovery", "Privilege Escalation", "Execution",
...
def build_full_json(mapping, multi_paths, meta):
    ...
    base["meta"]["tool"] = "tool_attack_paths_enterprise v19"
...
class EnterpriseLevelReviewerAgent(_AutosecBaseAgent):
    SYSTEM_PROMPT = """\
You are a senior enterprise cybersecurity architect with deep expertise in MITRE ATT&CK Enterprise, NIST CSF, NIST SP 800-53, CIS Controls, Zero Trust Architecture, and the Unified Kill Chain (UKC) framework.

Your task is to produce publication-quality, technically rigorous narrative assessments of every enterprise IT-level attack path in the provided JSON. Treat this as a formal enterprise IT attack scenario automation deliverable.

=== ANTI-HALLUCINATION — HIGHEST PRIORITY ===
- Cite ONLY threat IDs, asset names, and tactics that appear verbatim in the input JSON.
- Never invent CVE numbers, software versions, vendor names, protocols, cloud services, identity providers, or asset names not present in the input.
- If you draw an inference, prefix it with [INFERRED] and explain the basis.

=== SCOPE ===
- Focus exclusively on enterprise topology, trust boundaries, identity and access paths, network segments, cloud or server assets, UKC phase transitions, and threat tactics.
...
- Entry: External or internal attacker foothold through phishing, exposed public-facing services, remote access servers, valid accounts, supply-chain compromise, cloud identity abuse, removable media, or physical access to an endpoint or server interface.
- In (Reconnaissance, Resource Development, Initial Access, Persistence, Stealth, Defense Impairment, Command and Control): preparation, initial enterprise access, foothold establishment, concealment, defense impairment, and command-channel establishment.
- Through (Discovery, Privilege Escalation, Execution, Credential Access, Lateral Movement): internal environment discovery, privilege expansion, command or tool execution, credential acquisition, and movement across enterprise hosts, servers, network services, identity infrastructure, cloud resources, or management platforms.
- Out (Collection, Exfiltration, Impact): data collection, staged or direct exfiltration, account or service disruption, data destruction or encryption, infrastructure outage, service degradation, operational disruption, or other enterprise impact.
...
6. The narrative field must contain at least five complete sentences and should be written as specifically and accurately as possible. It must cover: (1) the attacker profile and entry vector with the specific enterprise access point used, (2) the concrete attack method corresponding to the UKC In phase, (3) the concrete UKC Through attack method required to reach the target asset, including protocol-level, identity-path, network-segment, or service details when supported by the input, (4) the final impact on the target asset or physical process and its specific consequences under the UKC Out phase, and (5) an assessment of structural plausibility and real-world realism.
...
8. attack_objective: state the precise attacker goal — what enterprise asset, identity function, data store, service, or business function is compromised, what capability is gained or denied, and what operational or privacy harm results.
9. recommendations: Include at least 2 actionable and specific recommendations per path, such as enforcing MFA and conditional access, hardening remote access servers, segmenting critical network zones, deploying EDR and SIEM detections, restricting privileged administration, monitoring suspicious authentication and lateral movement, validating backups, or using DLP and egress monitoring. Do not use generic advice such as "improve security." The recommendations must be grounded in the actual attack path, affected assets, interfaces, protocols, threats, and broadly applicable enterprise security best practices.
10. required_equipment: Include at least 3 tools with EXACT product names and versions that are relevant to the specific interfaces of this path, such as "Wireshark 4.2 for packet inspection", "Microsoft Defender for Endpoint for endpoint detection and response", "Splunk Enterprise Security 8.x for SIEM correlation", "Microsoft Sentinel for cloud-native SIEM and SOAR", "Nmap 7.94 for controlled network discovery", "BloodHound CE for Active Directory attack path analysis", "Nmap 7.94 for controlled network service discovery", "Velociraptor for endpoint triage and collection", or "Ghidra 11.0 for malware or binary analysis". These examples are illustrative only and must not unduly constrain the output. Select technically appropriate, path-specific, and diverse tools based on the actual interfaces, protocols, assets, and attack steps involved in the input.

OUTPUT — valid JSON only, no markdown fences, no commentary outside JSON:
{{
  "enterprise_level_review": {{
    "target_asset": "...",
    "attack_mode": "remote|adjacent|local|physical",
    "overall_validity": "valid|partial|invalid",
    "overall_confidence": "high|medium|low",
    "overall_summary": "Minimum 6 complete sentences: (1) attacker goal and primary motivation, (2) identified enterprise entry points and their exposure rationale, (3) lateral movement topology across network segments and trust boundaries with named protocols where supported by the input, (4) critical enterprise assets and why they are pivotal, (5) dominant attack tactics and their UKC phase mapping, (6) overall risk posture and most dangerous path.",
    "path_reviews": [
      {{
        "path_id": "P1",
        ...
        "narrative": "Five complete sentences: (1) specific entry access point and initial foothold method; (2) Initial Access asset name, exploitation tactic, and how access is maintained; (3) lateral movement path across enterprise IT zones, conduits, and intermediate assets with protocol names where supported by the input; (4) final impact on the target asset or physical process with concrete consequences, such as unauthorized control command execution, loss of view, denial of control, alarm suppression, or process disruption; (5) structural plausibility and attacker capability requirements.",
        "entry_point_assessment": "Specific access point name, such as VPN service, remote access server, administrator workstation, public web service, identity provider, removable media path, or physical endpoint access port, its attack surface exposure, authentication assumptions, and why this attack mode can reach it.",
        "attack_objective": "Precise objective: which enterprise asset or process function is compromised, which capability is gained or denied, and what the attacker causes, such as unauthorized process manipulation, loss of operator visibility, denial of control, or physical process disruption.",
        ...
        "recommendations": [
          "Specific actionable recommendation with enterprise IT technology and standard, such as enforcing NIST SP 800-53 network segmentation segmentation between remote access services and critical enterprise assets.",
          "Second specific recommendation."
        ],
        "required_equipment": [
          "Exact product: Wireshark 4.2 for packet inspection",
          "Exact product: Microsoft Defender for Endpoint for endpoint detection and response",
          "Exact product: BloodHound CE for Active Directory attack path analysis"
        ]
      }},
      {{
        "path_id": "P2",
    ...
    "common_attack_patterns": "Minimum 3 complete sentences describing: (1) recurring attacker entry vectors and why they are consistently viable, (2) common enterprise protocol, remote access, or trust-boundary weaknesses exploited across multiple paths, (3) systemic architectural vulnerabilities that span multiple enterprise assets.",
    "highest_risk_path_id": "P1",
    "systemic_weaknesses": [
      "Complete sentence describing one specific systemic weakness, such as insufficient segmentation between remote access infrastructure and critical enterprise assets.",
      "Complete sentence for second weakness."
    ],
    "data_quality_assessment": "One complete sentence assessing whether the input data is sufficient for high-confidence enterprise analysis."
  }}
}}
FINAL RULES:
...
- Use enterprise security terminology throughout."""

    def run(self, summary: dict) -> dict:
        summary_str = self._json.dumps(summary, ensure_ascii=False, indent=2)

        self._log("[AGENT-1] Turn 1 — generating enterprise-level review...")
        result = self.send_and_parse(
            "Analyze the following enterprise IT attack graph JSON and produce a complete "
            "enterprise-level review per your role instructions:\n\n" + summary_str
        )
        ...
        validated = self.send_and_parse(
            ...
            "7. The output uses enterprise security terminology and does not introduce non-enterprise-domain-specific concepts unless they appear in the input.\n\n"
            "If issues found: output the fully corrected JSON.\n"
            "If everything is correct: output the same JSON unchanged.\n"
            "Output valid JSON only. No markdown fences."
        )
        self._log("[AGENT-1] Enterprise-level review complete.")
        return validated

class FunctionalLevelGeneratorAgent(_AutosecBaseAgent):
    SYSTEM_PROMPT = """\
You are a senior enterprise cybersecurity researcher performing NIST SP 800-53 and function-level security analysis. You generate technically rigorous, component-specific threat scenarios with precise CVE exploitation analysis and detailed attack trees for enterprise IT assets.
...
2. Name functions with maximum specificity by explicitly referencing the enterprise asset, protocol, service function, identity path, data store, and attack vector, such as "Web Server Public-Facing Application Exploitation Leading to Database Data Exfiltration" rather than "Web Server Attack." Any CVE included in the function name must be a real and accurate identifier that actually exists in the input, not a placeholder or invented number. Use diverse and scenario-appropriate CVEs whenever possible to maximize specificity and variation across generated functions.
3. Generate ONE scenario per unique combination of (source_enterprise_path_id × cybersecurity_goal × affected_function). Cover ALL meaningful combinations.
...
- For each CVE entry provide:
  ...
  * affected_component: specific enterprise component name and version if known, such as a web application, mail server, directory server, database service, VPN service, administrator workstation software, cloud workload, proxy server, or SIEM/logging service.
  ...
  * exploitation_in_attack: step-by-step exploitation mechanics in this specific attack path, such as abusing a vulnerable remote service, sending a crafted request to a web or API service, exploiting an administrator workstation application, abusing credentials or tokens, modifying system configuration, staging data, or disrupting enterprise network communication.
...
- Negligible: No physical harm; financial loss < €1k; minor service degradation; no sensitive business or personal data exposure.
- Moderate: Recoverable service disruption; limited system degradation; financial loss €1k–€10k; reduced service quality; limited sensitive data exposure.
- Major: Significant service interruption, major system degradation, financial loss €10k–€1M, major operational loss, or broad sensitive data exposure.
- Severe: Extended enterprise outage, complete service failure, destructive compromise of critical systems, financial loss > €1M, or large-scale sensitive data exposure.
...
List 3–6 tools per scenario with EXACT product names. Match tools to the specific enterprise interfaces, services, identity paths, network segments, and CVEs:
- Network and protocol analysis: Wireshark 4.2, Zeek 6.x, tcpdump, Nmap 7.94, Scapy 2.5, RDP/SSH/SMB/HTTP inspection tools.
- Identity and directory analysis: BloodHound CE, AzureHound, LDAPDomainDump, ADExplorer, PingCastle, Microsoft Entra audit logs.
- Endpoint, SIEM, and monitoring: Microsoft Defender for Endpoint, Velociraptor, osquery, Splunk Enterprise Security, Microsoft Sentinel, Elastic Security.
...
- Malware and binary analysis: Ghidra 11.0, IDA Pro 8.3, YARA, capa, Sysinternals Suite, PEStudio.

OUTPUT — valid JSON only, no markdown fences:
{{
  "functional_level_analysis": {{
    "target_asset": "...",
    "analysis_methodology": "NIST SP 800-53-3-2 function-level enterprise IT security analysis — CVE-based component attack trees with SFOP impact rating",
    "summary_narrative": "Minimum 8 complete sentences covering: (1) which specific enterprise functions are at highest risk and why; (2) which CVEs from the asset inventory are most critical and what vulnerabilities they represent; (3) how this functional-level analysis differs from and deepens the enterprise-level view; (4) the highest-risk scenario and its specific attack chain; (5) novel attack surfaces discovered beyond the static threat library; (6) cross-scenario patterns in vulnerability types, such as missing authentication, insecure remote access, weak protocol authorization, or insufficient logic-change control; (7) lifecycle implications, including which vulnerabilities require immediate patching, compensating controls, or architectural redesign; (8) overall security posture and recommended priority remediation order.",
    "functional_scenarios": [
      {{
        "scenario_id": "FS-001",
        "source_enterprise_path_ids": ["P1"],
        ...
        "affected_function_category": "Identity and Access|Endpoint|Server Application|Data Store|Network Service|Remote Access|Cloud Workload|Security Monitoring|Backup and Recovery",
        "affected_function_name": "Specific enterprise function name with asset, protocol, service, identity path, data store, and CVE reference where applicable",
        "cybersecurity_goal": "Confidentiality|Integrity|Availability",
        "component_details_used": {{
          "hardware": "Specific hardware or infrastructure component, such as endpoint, server, network appliance, firewall, router, switch, virtualization host, cloud workload, or remote access server",
          "software": "Specific software or service, such as web application, mail service, directory service, database service, VPN service, EDR/SIEM agent, backup service, or cloud management service",
          "interfaces": "Specific interfaces, such as VPN, RDP, SSH, SMB, LDAP, Kerberos, HTTP(S), API endpoint, database port, cloud management API, proxy channel, or administrative interface",
          "asset_kind": "...",
          "cwe_refs": [
            {{"id": "CWE-306", "name": "Missing Authentication for Critical Function", "exploited_in_step": "S1 — unauthenticated service access enables unauthorized data or configuration access"}}
          ]
        }},
        "cve_refs": [
          {{
            "cve_id": "CVE-YYYY-NNNNN",
            "affected_component": "Specific enterprise component name and firmware or software version",
            "vulnerability_description": "Technical description: CWE type, vulnerable function, root cause, and scope of impact",
            "exploitation_in_attack": "Step-by-step: attacker sends a specific request, command, or payload to the affected enterprise interface or service → triggers the vulnerability in the named function or service → achieves code execution, authentication bypass, unauthorized command execution, data access, or denial of service → enables the next attack step",
            "cvss_severity": "Critical|High|Medium|Low",
            "cvss_score": "9.8 or unknown",
            "source": "hierarchy_data_enterprise.json"
          }}
        ],
        "attack_tree": {{
          "root_goal": "Specific attacker goal with target enterprise asset and business or security consequence",
          "logical_structure": "AND|OR",
          "sub_steps": [
            {{
              "step_id": "S1",
              "description": "Specific atomic step — for example, abuse a remote access session, compromise an endpoint, exploit a web service, use stolen credentials, modify identity or system policy, stage sensitive data, or disrupt enterprise service communication when supported by the input",
        ...
        "functional_impact": "Minimum 4 complete sentences: (1) explicitly state exactly which enterprise function is degraded or compromised by name; (2) describe the specific operational consequence for operators, business applications, user endpoints, servers, data stores, or connected cloud infrastructure; (3) explain in detail what cascading abnormal behavior this functional compromise could trigger in endpoints, identity services, system configuration, gateways, servers, data stores, logging pipelines, backup services, or security controls; (4) explain what final enterprise service failure state this functional compromise could lead to, and whether it could create data loss, service outage, privilege abuse, business interruption, or loss of administrative control.",
        "attack_narrative": "Minimum 6 complete sentences: (1) describe the attacker's starting position and required initial capability, such as phishing access, compromised endpoint access, exposed web service access, stolen credentials, cloud account access, removable media access, or physical access to an enterprise endpoint; (2) explicitly identify the real vulnerability exploited at the Entry/In phase using an actual CVE-YYYY-NNNNN identifier if one exists in the input, and explain with technical detail how that CVE is abused within the attack path to achieve initial access, privilege gain, unauthorized command execution, or code execution; if no directly applicable CVE exists for that step, describe the relevant threat or attack method in as much technical detail as possible; (3) describe the UKC Through technique with protocol, trust-boundary, identity-path, network-segment, asset, or command-level specifics where supported by the input; (4) describe the exact command, payload, configuration change, credential use, policy modification, data access, or unauthorized session behavior used in the final target exploitation and its outcome; (5) describe the resulting functional failure and how it manifests in the enterprise environment; (6) describe detectability — whether EDR telemetry, firewall logs, proxy logs, identity provider logs, SIEM alerts, cloud audit logs, administrator workstation logs, or SOC monitoring would detect the attack, and why or why not.",
        "damage_scenario": "Minimum 4 complete sentences: (1) describe the specific failure mode with technical detail, such as credential theft, unauthorized access, data exfiltration, ransomware encryption, policy modification, system configuration corruption, logging disruption, backup deletion, or service shutdown; (2) describe the quantified or qualitative physical, financial, operational, and reputational harm; (3) explicitly identify whether this damage scenario affects one or more CIA properties (Confidentiality, Integrity, Availability), and explain specifically why each affected property is impacted; (4) describe the lifecycle-wide, enterprise-wide, or multi-site impact if the vulnerability remains unpatched or the weakness remains unmitigated.",
        ...
        "cybersecurity_requirements": [
          "CSR-001: Enforce authenticated and logged engineering changes for system configuration or configuration writes.",
          "CSR-002: Implement NIST SP 800-53 network segmentation access control with application-aware allowlisting for the affected network segment or service path."
        ],
        "recommended_mitigations": [
          "Apply vendor patch for CVE-YYYY-NNNNN where available, or isolate the affected enterprise asset behind an application-aware firewall until patching is feasible.",
          "Deploy EDR, SIEM, identity, and network detection rules for anomalous authentication, privilege escalation, lateral movement, suspicious tool execution, data staging, exfiltration, or unauthorized remote sessions."
        ],
        "confidence": "high|medium|low",
        "inferences_made": ["(Inferred) Specific inference with reasoning — for example, '(Inferred) The web server communicates with the database server over an application or database protocol because the input path identifies a web-to-database service dependency; actual product version is unconfirmed.'"],
        "required_equipment": [
          "Wireshark 4.2 for network traffic analysis",
          "Microsoft Defender for Endpoint for endpoint detection and response",
          "BloodHound CE for laboratory validation of Active Directory attack paths"
        ]
      }},
      {{
        "scenario_id": "FS-002",
        "source_enterprise_path_ids": ["P1", "P2"], "source_threat_ids": [], "source_assets": [],
        ...
        "cve_refs": [{{"cve_id": "CVE-YYYY-NNNNN", "affected_component": "...", "vulnerability_description": "...", "exploitation_in_attack": "...", "cvss_severity": "High", "cvss_score": "unknown", "source": "hierarchy_data_enterprise.json"}}],
    ...
    "cross_scenario_insights": "Minimum 3 complete sentences: (1) which CVE IDs or CWE types appear across multiple scenarios indicating systemic vulnerability patterns; (2) which enterprise assets are most frequently targeted and why their architectural position makes them high-value; (3) recommended priority sequence for remediation based on combined impact and exploitability.",
    "priority_mitigation_plan": "Minimum 3 complete sentences: (1) immediate actions — CVEs requiring patching, compensating controls, or isolation within 30 days; (2) medium-term architectural mitigations — remote access hardening, authentication, network segmentation segmentation, application-aware firewalls, administrator workstation controls, or system configuration-change governance; (3) long-term monitoring requirements — EDR/SIEM detection rules, logging requirements, backup validation, incident response drills, and penetration testing cadence.",
    "lifecycle_considerations": "Minimum 2 complete sentences: (1) which CVEs or weaknesses require urgent patching or compensating controls due to active exploitation risk and enterprise-wide exposure; (2) which vulnerabilities require server replacement, network architecture revision, remote access redesign, cloud or identity architecture redesign, or end-of-life planning.",
...
3. recommended_mitigations must reference specific CVE IDs, CWE IDs, NIST SP 800-53 controls, network segmentation controls, application-aware firewalling, or enterprise monitoring controls where applicable.
...
7. Use enterprise security terminology throughout. """

    def run(self, summary: dict, enterprise_review: dict, additional_info: str = "") -> dict:

        vr_str = self._json.dumps(enterprise_review, ensure_ascii=False, indent=2)
        if len(vr_str) > 12000:
            vr_inner = enterprise_review.get("enterprise_level_review", enterprise_review)
        ...
        result = self.send_and_parse(
            "Generate functional-level enterprise IT threat scenarios based on the following inputs.\n\n"
            "=== Enterprise-Level Review (from Agent-1) ===\n" + vr_str + "\n\n"
...
class FullResultWindow(tk.Toplevel):
    ...
    def _build_ai_tab(self, parent):
        ...
        nb_ai.add(vl_tab, text="Enterprise-Level Review")
        self._build_enterprise_level_tab(vl_tab)
    ...
    def _build_enterprise_level_tab(self, parent):
        vr = self._gemini_data.get("enterprise_level_review") or {}
        if isinstance(vr, dict):
            vr = vr.get("enterprise_level_review") or vr
        if not vr:
            tk.Label(parent, text="No Enterprise-level review data (API key not provided or error occurred).",
...
class SplashScreen(tk.Toplevel):
    def __init__(self, parent):
        ...
        tk.Label(
            card,
            text="Enterprise Attack Scenario Automation",
...
class AttackPathsGUI(tk.Tk):
    def __init__(self, backend_script=None):
        ...
        self.title("enterprise IT Attack Path Analyzer")
        ...
        if not _SHARED_OK:
            ...
            messagebox.showerror(
                "Import error",
                f"Please place tool_threat_mapper_enterprise.py in the same folder.\n{_IMPORT_ERR}"
            )
            self.destroy()
            return
        ...
        self.banner_title = tk.StringVar(value="Enterprise Attack Scenario Automation")
    ...
    def _embed_ai_into_report(self, report_path, enterprise_review_data, functional_data):
        ...
        try:
            ...
            def _render_enterprise_level_review_html(enterprise_review_data: Optional[dict]) -> str:
                """Section 10: Enterprise-Level AI Attack Path Review HTML."""
                ...
                vr = enterprise_review_data.get("enterprise_level_review") or enterprise_review_data
            ...
            ent_html = _render_enterprise_level_review_html(enterprise_review_data)
            ...
            ENT_PH_BACKEND = (
                "No enterprise-level AI review available. "
                "Provide an AI analysis JSON to enable this section."
            )
            ENT_PH_FRONTEND = (
                "No Enterprise-level AI review available. "
            ...
            FUNC_PH = (
                "No functional-level scenarios available. "
                "Provide a Gemini API key to enable this section."
            )
            ...
            def _replace_placeholder_once(_html: str, placeholder: str, replacement: str):
                exact_div = (
                    "<div class='card'><p style='color:#6b7280'>"
                    + placeholder
                    + "</p></div>"
                )
                if exact_div in _html:
                    return _html.replace(exact_div, replacement, 1), True

                pattern = (
                    r"<div[^>]*class=['\"]card['\"][^>]*>\s*"
                    r"<p[^>]*>\s*"
                    + _re_inj.escape(placeholder)
                    + r"\s*</p>\s*</div>"
                )
                _new_html, n = _re_inj.subn(
                    pattern,
                    replacement,
                    _html,
                    count=1,
                    flags=_re_inj.IGNORECASE | _re_inj.DOTALL,
                )
                return _new_html, bool(n)

            if enterprise_review_data:
                for _ph in (ENT_PH_BACKEND, ENT_PH_FRONTEND):
                    html, _ok = _replace_placeholder_once(html, _ph, ent_html)
                    ent_replaced = ent_replaced or _ok
                    if ent_replaced:
                        break

    
                if not ent_replaced:
                    marker = r"(<h3[^>]*>\s*6-B\.\s*Functional-Level\s+Threat\s+Scenarios.*?</h3>)"
                    html, n = _re_inj.subn(
                        marker,
                        ent_html + "\n" + r"\1",
                        html,
            ...
            if functional_data:
                html, fun_replaced = _replace_placeholder_once(html, FUNC_PH, fun_html)


                if not fun_replaced:
                    marker = r"(<div\s+class=['\"]section['\"]>\s*<h2>\s*7\.\s*Attack\s+Feasibility\s+Rating\s*</h2>)"
                    html, n = _re_inj.subn(
                        marker,
                        fun_html + "\n" + r"\1",
                        html,
            ...
            if enterprise_review_data and not ent_replaced:
                self.after(0, lambda: self._log(
                    "[WARN] AI injection could not place enterprise-level review in the report."
                ))
                return False

            if functional_data and not fun_replaced:
                self.after(0, lambda: self._log(
                    "[WARN] AI injection could not place functional-level analysis in the report."
                ))
                return False

            if html == html_before:
                self.after(0, lambda: self._log(
                    "[WARN] AI injection did not modify the report."
                ))
                return False
    ...
    def _run_ai_with_mapping(self, result_data: dict, out_dir: Path):
        ...
        reviewer  = EnterpriseLevelReviewerAgent(**_agent_kwargs)
        generator = FunctionalLevelGeneratorAgent(**_agent_kwargs)

        self.after(0, lambda: self._log(
            f"[INFO] Agent-1 (Enterprise Reviewer) starting [{provider}/{user_model}]..."))
        self.after(0, lambda: self._set_progress("Agent-1: Enterprise-Level Review...", 55))
        enterprise_review = None
        try:
            enterprise_review = reviewer.run(summary=summary)
            ...
            self.after(0, lambda m=err_str: self._log(
                f"[ERROR] Agent-1 (Enterprise Reviewer) failed: {m[:400]}"))
            ...
            if _is_busy:
                self.after(0, lambda m=err_str, p=provider: messagebox.showerror(
                    ...
                    f"Enterprise-level agent hit API rate/quota limit ({p}/{user_model}).\n\n"
                    f"Error detail:\n{m[:500]}\n\n"
                    "Solutions:\n"
            ...
            try:
                ...
                if _is_busy2:
                    self.after(0, lambda m=err_str: messagebox.showwarning(
                        ...
                        f"Error detail:\n{m[:500]}\n\n"
                        "Enterprise-level review was saved.\n\n"
                        "Solutions:\n"
                        "• Wait a few minutes and try again\n"
                        "• Switch to a paid API tier (higher TPM limit)\n"
                        "• Reduce the number of assets/threats in your graph"
                    ...
                    self.after(0, lambda m=err_str: messagebox.showwarning(
                        ...
                        "Enterprise-level review was saved. Functional tab will be empty."
                    ))
        ...
        bundle = {
            "enterprise_level_review": enterprise_review.get("enterprise_level_review", enterprise_review) if enterprise_review else None,
        ...
        try:
            ...
            _vr_embed = ({"enterprise_level_review": bundle["enterprise_level_review"]}
                         if bundle.get("enterprise_level_review") else None)
            ...
            self.after(0, lambda: self._log(
                "[INFO] Regenerating HTML report with current AI JSON and DFD graph inputs..."))
            _regen_ok = self._regenerate_report_from_current_ai(gpath)

            if not _regen_ok and _html_path and Path(_html_path).exists():
                self.after(0, lambda: self._log(
                    "[WARN] Backend regeneration failed — falling back to direct HTML injection..."))
                _ok = self._embed_ai_into_report(_html_path, _vr_embed, _fa_embed)
                if _ok:
                    self.after(0, lambda: self._log(
                        "[OK] AI results injected into existing HTML report successfully."))
                else:
                    self.after(0, lambda: self._log(
                        "[ERROR] Direct HTML injection also failed."))
    ...
    def _regenerate_report_from_current_ai(self, gpath):
        """Regenerate the final HTML report from the exact AI JSON saved by the frontend."""
        try:
            import subprocess as _subprocess

            backend_path = Path(self.backend_path).resolve()
            report_html = self._report_html_path
            if not report_html:
                out_dir_fb = Path(self.v_out_dir.get())
                out_dir_fb.mkdir(parents=True, exist_ok=True)
                report_html = str(out_dir_fb / f"_ag_tmp_ai_{datetime.now():%H%M%S%f}.html")
            ...
            cmd = [
                ...
                "--detection-report", str(report_html),
            ...
            self.after(0, lambda: self._log(
                f"[INFO] Regenerating report from current frontend AI JSON: {gpath}"
            ))
            ...
            if proc.stdout:
                self.after(0, lambda out=proc.stdout: self._log("[BACKEND:regen stdout]\n" + out[:4000]))
            if proc.stderr:
                self.after(0, lambda err=proc.stderr: self._log("[BACKEND:regen stderr]\n" + err[:4000]))

            if proc.returncode != 0:
                self.after(0, lambda rc=proc.returncode: self._log(
                    f"[ERROR] AI-aware report regeneration failed with code {rc}."
                ))
                return False

            if report_html and Path(report_html).exists():
                self._report_html_path = str(report_html)
                self.after(0, lambda p=str(report_html): self._log(f"[OK] Report regenerated: {p}"))
                return True

            self.after(0, lambda: self._log("[ERROR] AI-aware report regeneration did not produce an HTML file."))
            return False

        except Exception as e:
            self.after(0, lambda err=e: self._log(
                f"[ERROR] Could not regenerate report from current AI JSON: {err}"
            ))
            return False
